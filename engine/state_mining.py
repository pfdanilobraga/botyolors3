...
class MiningFSM:
    ...
    def __init__(self, 
        ...
        self.machine.add_transition('rocha_encontrada', 'PROCURANDO_ROCHA', 'MOVENDO_PARA_ROCHA')
        self.machine.add_transition('movimento_concluido', 'MOVENDO_PARA_ROCHA', 'MINERANDO')
    ...
    def on_enter_MOVENDO_PARA_ROCHA(self):
        ...
        if self.target_coords_abs:
            ...
            print(f"[{time.strftime('%X')}] Estado: MOVENDO_PARA_ROCHA - Clicando em ({x}, {y})")
            ...
            print(f"[{time.strftime('%X')}] Erro: Coordenadas do alvo não encontradas. Resetando.")
            self.mineracao_parada()

    def on_enter_MINERANDO(self):
        print(f"[{time.strftime('%X')}] Estado: MINERANDO - Monitorando Pirâmide de Prioridade.")

    def on_enter_INVENTARIO_CHEIO(self):
        print(f"[{time.strftime('%X')}] Estado: INVENTARIO_CHEIO - Ciclo interrompido. Mochila cheia.")
    ...
    def update(self, frame: np.ndarray):
        ...
        if current_state in ['IDLE', 'PROCURANDO_ROCHA', 'MINERANDO']:
            ...
            if self.inventory.is_full(frame):
                if current_state != 'INVENTARIO_CHEIO':
                    print(f"[{time.strftime('%X')}] [Segurança] Inventário detectado como CHEIO.")
        ...
        if current_state == 'MINERANDO':
            ...
            if agora - self.ultimo_check_mochila > 2.0:
                self.ultimo_check_mochila = agora
                if self.inventory.is_full(frame):
                    print(f"[{time.strftime('%X')}] [P1] Inventário encheu! Abortando mineração.")
                    self.inventario_lotado()
                    return 
            ...
            if not self.validator.esta_minerando(frame):
                print(f"[{time.strftime('%X')}] [P2] Rocha esgotada ou indicador não visível. Buscando novamente...")
                self.mineracao_parada()
                return 
            ...
            if targets:
                ...
                if win:
                    self.target_coords_abs = (best['x'] + win['left'], best['y'] + win['top'])
                    print(f"[{time.strftime('%X')}] Alvo encontrado: {self.target_class} ({best['conf']:.2f})")
